...
**أداة احترافية لتوليد وتحويل الأيقونات لجميع المنصات (CLI وGUI) — يدعم العربية والإنجليزية**
...
- 🆕 **وضع batch غير تفاعلي** للـ CLI — 5 علامات تتخطى الواجهة التفاعلية (ممتاز لـ CI/CD والاستدعاء البرمجي).
...
- ✅ **38 حجم أيقونة** (من 16px إلى 1024px) — يغطي 8 منصات
- ✅ **واجهتان** (سطر الأوامر CLI والواجهة الرسومية GUI)
- ✅ **i18n**: العربية والإنجليزية — كشف تلقائي من `LANG` أو `--lang=ar|en`
- ✅ **وضع batch + JSON** للاستدعاء البرمجي والـ CI
...
> **بعد التثبيت:** إذا لم تظهر الأيقونات في قائمة البرامج فوراً، نفّذ `gtk-update-icon-cache ~/.local/share/icons/hicolor` أو سجّل خروج وعد للدخول.
...
```bash
gt-iconscaler                    # كشف تلقائي للغة من LANG
gt-iconscaler --lang=en          # فرض الإنجليزية
GT_LANG=ar gt-iconscaler         # عبر متغير البيئة
gt-iconscaler --help             # عرض المساعدة
gt-iconscaler --version          # عرض الإصدار
```
ستظهر لك واجهة تفاعلية تطلب:
1. **اختيار ملف الأيقونة** (PNG, SVG, JPG, WebP)
2. **اختيار مجلد التصدير**
3. **تحديد اسم الحزمة**
4. **اختيار المنصات المطلوبة** (Linux, PWA, Android, iOS, watchOS, Windows, Electron)
5. **اختيار نوع الضغط** (ZIP, tar.gz, أو بدون ضغط)

### 🎨 النسخة الرسومية (GUI)
```bash
gt-iconscaler-gui                # كشف تلقائي للغة
gt-iconscaler-gui --lang=en      # فرض لغة
```
نافذة رسومية بـ zenity توجّهك عبر جميع الخطوات.
...
|---------|--------|------|
| 1 | `--lang=` | `--lang=en` |
| 2 | `GT_LANG` env | `GT_LANG=ar` |
| 3 | `LANG` env | `LANG=ar_SA.UTF-8` |
| 4 | الافتراضي | `en` |

### ⚙️ وضع batch (CLI فقط — جديد في v2.2)

تخطّى الواجهة التفاعلية بتمرير كل المعاملات مرة واحدة — مناسب للـ CI/CD والاستدعاء البرمجي.
...
**`--json`** يُخرج سطر JSON واحد (يكتم الديكور — مناسب للاستدعاء من برنامج):
```bash
$ gt-iconscaler --json --icon=logo.png --output=/tmp --name=app \
                --platforms=all --compress=zip
{"final_dir":"/tmp/app","done":38,"failed":0,"has_ico":true,"has_icns":true,"icns_method":"imagemagick","compress":"zip","archives":["/tmp/app.zip"]}
```

أي علم مفقود يرجع لخطوته التفاعلية تلقائياً — الوضع الهجين مدعوم.
...
| **PWA** | `pwa/` | Web Manifest + Apple Touch + Chrome Android | 16, 32, 48, 72, 96, 128, 144, 152, 167, 180, 192, 196, 256, 384, 512 |
| **Android** | `android/` | mipmap-ldpi → mipmap-xxxhdpi | 36, 48, 72, 96, 144, 192 |
| **iOS** | `ios/` | App Icon (iPhone, iPad, App Store) | 20, 29, 40, 58, 60, 76, 80, 87, 100, 120, 152, 167, 180, 512, 1024 |
| **watchOS** | `watchos/` | Apple Watch (38mm/44mm) | 48, 55, 58, 88, 100, 172, 196, 216 |
| **Windows Tiles** | `windows/` | MS Store + Win10/11 Tiles (PWA/MSIX) | 44, 50, 70, 150, 310 |
| **Windows ICO** | `.ico` | ملف ICO متعدد الأحجام | 16, 24, 32, 48, 64, 128, 256 |
...
│   ├── Icon-29.png
│   └── ...
├── watchos/                  # أيقونات Apple Watch
│   ├── AppIcon-48.png
│   ├── AppIcon-88.png
│   └── ...
├── windows/                  # Microsoft Tiles (Win10/11)
│   ├── StoreLogo-44.png
│   ├── Square70x70Logo.png
...
استخدم السكربت الجاهز — يحذف الملفات التنفيذية، وملفات `.desktop`، وجميع مقاسات الأيقونات، ثم يحدّث ذاكرة التخزين المؤقت:

```bash
./uninstall.sh        # لإزالة التثبيت المحلي
# أو
sudo ./uninstall.sh   # لإزالة التثبيت النظامي
```
